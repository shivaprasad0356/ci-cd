...
resource "aws_codepipeline" "pipe" {
  ...
  stage {
    ...
    action {
      ...
      version = "1"
      input_artifacts = ["bin"]
      configuration = { BucketName = aws_s3_bucket.app.bucket, Extract = "true" }
    }
  }
}
...
resource "aws_iam_role" "role" {
  name = "pip-automation-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17", Statement = [{
      Action = "sts:AssumeRole", Effect = "Allow", Principal = { Service = ["codepipeline.amazonaws.com", "codebuild.amazonaws.com"] }
    }]
  })
}
...
output "app_url" { value = aws_s3_bucket_website_configuration.web.website_endpoint }
